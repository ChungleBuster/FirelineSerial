...
if ! compile_with_retry; then
    log_retry "make all failed after $MAX_ATTEMPTS attempt(s); cannot continue."
    exit 1
fi

{
    ...
    echo "# To recreate any row by hand, run the exact command in the 'command' column below (from src/, after 'make all')"
...
run_one() {
    ...
    if ! out="$(run_simulation_with_retry "$cmd")"; then
        log_retry "Skipping $impl n=$n seed=$seed — no row written."
        return 0
    fi
